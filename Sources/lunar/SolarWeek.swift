...
struct SolarWeek {
    ...
    init(date: Date = Date(), start: Int = 0) {
        let dateCompment: DateComponents = Calendar(identifier: .gregorian).dateComponents([.year, .month, .day], from: date)
        self.year = dateCompment.year!
        self.month = dateCompment.month!
        self.day = dateCompment.day!
        self.start = start
    }
    ...
    if (separateMonth) {
        var n: Int = weeks
        var c: Date = DateComponents(calendar: Calendar(identifier: .gregorian),year: year,month: month, day: day).date!
